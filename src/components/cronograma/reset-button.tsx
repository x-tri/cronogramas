...
export function ResetButton({ variant = 'default' }: { variant?: 'default' | 'icon' } = {}) {
    ...
    <>
      {variant === 'icon' ? (
        <div className="group/reset relative">
          ...
          <div className="pointer-events-none absolute left-1/2 top-full z-30 mt-2 w-40 -translate-x-1/2 rounded-xl bg-[#0f172a] px-3 py-2 text-center text-xs text-white opacity-0 shadow-lg transition-all duration-150 group-hover/reset:opacity-100">
            Refazer semana
          </div>
        </div>
      ) : (
